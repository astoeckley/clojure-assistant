...
(deftest predicates
  ...
  (testing "nilables"
    (is (false? (nil-or? integer? :hi)))
    (is (nil-or? integer? 4))
    (is (false? (nil-or? #(count %) :hi)))
    (is (true? (nil-or? integer? (when false 5))))
    (is (false? ((nilable integer?) "hi")))
    (is ((nilable integer?) (inc (+ 5 6))))
    (is ((nilable #(map-structure? string? keyword? %)) {"hi" :hi "there" :there}))
    (is (false? ((nilable #(map-structure? string? keyword? %)) {"hi" :hi "there" "there!"})))))
